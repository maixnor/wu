...
# rolling estimation using 10*12 months (10 years), advancing by 12 months at each iteration
betas <- rollapply(cbind(r[,stock_picks], r_mkt)-c(coredata(rf)), width=10*12, by=12, fill=NULL, align="right", by.column=FALSE,
                   FUN=function(x){
                     beta <- rep(NA_real_, 3)
                     for(i in 1:3){
                       beta[i] <- coef(lm(x[,i]~x[,4]))[2] #column 4 contains the market returns
                     }
                     return(beta)
                   } )
...
## run regressions over two-year windows (24 months)
betas <- rollapply(data = xret_data, width = 24, by.column = FALSE, FUN = function(x){
  #print((x))
  b <- list()
  y <- as.matrix(x[,2:13]) # each column corresponds to one industry
  X <- cbind(rep(1,24), as.matrix(x[,14:16])) # regressors: mkt excess returns, SMB and HML reteurns
  for(i in 1:12) b[[i]] <- solve(t(X)%*%X)%*%t(X)%*%y[,i] #linear regression coefficient estimates
